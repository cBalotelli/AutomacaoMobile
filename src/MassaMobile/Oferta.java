...
ArrayList<String> ListaOfertas = new ArrayList<String>();
                    ...
                    WebElement ListaTabelaX = navegador.findElement(By.xpath("//table[2]//tr[3]//td[1]"));
                    List<WebElement> ListaTabela = ListaTabelaX.findElements(By.xpath("//tr[@style='cursor:hand']")); 




 JOptionPane.showMessageDialog(null,"Aguarde"+ListaTabela.size());
//
 WebElement Element = null;
//for(int X =5 ; X< ListaTabela.size(); X++){ // lista de 3478
// do{
//    try{
////     Element = wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//td[@class='bgTable']//tr["+X+"]//input[@type='checkbox']"))); 
//      Element = navegador.findElement(By.xpath("//td[@class='bgTable']//tr["+X+"]//input[@type='checkbox']"))); 
//  
//     break;
// }catch(Exception A){
//     X++;
...
for(int X =5 ; X< ListaTabela.size(); X++){ 
     
  try{
   WebElement a = navegador.findElement(By.xpath("//tr[@style='cursor:hand']["+X+"]//img[@src='../images/plus.gif']")); 
   }catch(NoSuchElementException A){

      ListaOfertas.add(Integer.toString(X));
      List<WebElement> listaBox =  navegador.findElements(By.xpath("//tr[@style='']//input[@type='checkbox']"));
       
      
      
      
//   esta certo;
//   criar os array no transfers.
           
           
           
           
       int z= 0;
           for(WebElement Box: listaBox){
      z++;
               try{
         box =  Box.isSelected(); 
            
         if(box == true){
              JOptionPane.showMessageDialog(null, "dentro do if: "+ z);
              break;
         }
